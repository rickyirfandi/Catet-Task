...
#[derive(Parser)]
#[command(name = "catet-cli", about = "Catet Task CLI — time tracking for Jira", version)]
struct Cli {
...
enum Commands {
    ...
    Report {
        #[arg(long, default_value = "standup", help = "Output format: standup, json, csv")]
        format: String,
    },

    /// Install this binary to PATH (creates symlink at ~/.local/bin/catet-cli)
    Install {
...
async fn run(cli: Cli, db_path: &PathBuf) -> Result<(), String> {
    match cli.command {
        Commands::Status => {
            ...
            if cli.json {
                match &session {
                    None => println!("{}", json!({"status": "idle", "task_id": null, "elapsed_secs": 0})),
                    Some(s) => {
                        let state = format::session_to_state(s);
                        // If running, look up summary
                        let summary = if let Some(ref task_id) = state.task_id {
                            db::get_task(&pool, task_id).await.ok().flatten().map(|t| t.summary)
                        } else { None };
                        println!("{}", json!({
                            "status": state.status,
        ...
        Commands::Range { from, to } => {
            ...
            if cli.json {
                let aggregated = format::aggregate_entries(&entries, &task_map);
                println!("{}", serde_json::to_string_pretty(&json!({"entries": entries, "aggregated": aggregated})).unwrap());
            } else {
                format::print_week(&entries);
            }
        }
        ...
        Commands::Entries { unlogged, running, date } => {
            let pool = db::open_pool(db_path).await?;
            ...
            let filtered: Vec<_> = entries.into_iter().filter(|e| {
                if running { return e.end_time.is_none(); }
                if unlogged { return !e.synced_to_jira && e.end_time.is_some(); }
                true
            }).collect();
            let tasks = db::get_all_tasks(&pool).await?;
            let task_map: HashMap<String, &models::TaskRow> =
        ...
        Commands::SetComment { entry_id, comment } => {
            ...
            let entry = db::get_entry(&pool, entry_id).await?
            ...
            if cli.json {
                ...
                println!("{} Comment set on entry {} ({})", "✓".green(), entry_id, entry.task_id.cyan());
            }
        }
        ...
        Commands::SetDuration { entry_id, duration } => {
            ...
            let entry = db::get_entry(&pool, entry_id).await?
                .ok_or(format!("Entry {} not found", entry_id))?;
            db::update_entry(&pool, entry_id, Some(adjusted_secs), entry.description.as_deref()).await?;
            if cli.json {
                println!("{}", json!({"ok": true, "entry_id": entry_id, "adjusted_secs": adjusted_secs}));
            } else {
                println!("{} Duration set to {} on entry {} ({})", "✓".green(), format::fmt_duration(adjusted_secs), entry_id, entry.task_id.cyan());
            }
        }

        Commands::Submit { entry, all_unlogged, include_running } => {
            let pool = db::open_pool_rw(db_path).await?;
            ...
            if entries.is_empty() {
                if cli.json {
                    println!("{}", json!({"results": [], "message": "No entries to submit"}));
                } else {
            ...
            for entry in &entries {
                if entry.is_running() && !include_running {
                    if !cli.json {
                        println!("{} {} {} skipped (timer still running)", "○".yellow(), entry.task_id.cyan(), format::fmt_duration(entry.effective_secs()));
                    }
                ...
                if !cli.json {
                    print!("  Submitting {} {} ... ", entry.task_id.cyan(), format::fmt_duration(entry.effective_secs()).yellow());
                }
            ...
            if cli.json {
                println!("{}", serde_json::to_string_pretty(&json!({"results": results})).unwrap());
            } else {
                let logged = results.iter().filter(|r| r["status"] == "logged").count();
                let skipped = results.iter().filter(|r| r["status"] == "skipped").count();
                let errors = results.iter().filter(|r| r["status"] == "error").count();
                println!();
                println!("Summary: {} logged, {} skipped, {} errors", logged.to_string().green(), skipped.to_string().yellow(), errors.to_string().red());
            }
        }
        ...
        Commands::Report { format } => {
            ...
            match format.as_str() {
                ...
                "json" | _ => {
                    ...
                    fn summarize<'a>(entries: &[models::EntryRow], tasks: &HashMap<String, &'a models::TaskRow>) -> Vec<serde_json::Value> {
                        let mut by_task: indexmap::IndexMap<String, i64> = indexmap::IndexMap::new();
                        for e in entries { *by_task.entry(e.task_id.clone()).or_insert(0) += e.effective_secs(); }
        ...
        Commands::Install { target } => {
            ...
            let link_path = target_dir.join("catet-cli");

            // Remove existing symlink/file if present
            if link_path.exists() || link_path.symlink_metadata().is_ok() {
                std::fs::remove_file(&link_path)
                    .map_err(|e| format!("Cannot remove existing {}: {}", link_path.display(), e))?;
            }
            ...
            println!("{} Installed to {}", "✓".green(), link_path.display().to_string().cyan());
            println!("  Make sure {} is in your PATH.", target_dir.display().to_string().yellow());
        }
        ...
        Commands::ConnectClaude => {
            ...
            mcp_servers.insert("catet-task".to_string(), json!({
            ...
            std::fs::write(&config_path, output)
                .map_err(|e| format!("Cannot write Claude Desktop config: {}", e))?;
